...
main:                     # Label que define o início do código do usuário
	...
	lbu $t0, 0($s0)   # Copia o byte a posição de memória [$s0+0] p/ $t0
	sb $t1, 1($s0)    # Copia o byte menos significativo (à direita) de $t1 na posição de memória indicada por [$s0+1]
